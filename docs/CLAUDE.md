...
**KAN-39 pre-deploy env-var assertion (added 2026-05-04):** `backend-deploy.yml` now includes a "Assert required Cloud Run env vars are present" step that runs before the Docker build on every push to main. It calls `gcloud run services describe --format=json`, pipes the output through `jq` to check that `DOCUMENT_AI_PROCESSOR_ID`, `GEMINI_API_KEY`, `GEMINI_MODEL`, and `GCS_BUCKET_NAME` are all present with non-empty values on the current revision, and fails the workflow with a descriptive `::error::` annotation if any are missing. This would have caught the 2026-04-21 `/analyze` outage at promotion time instead of on the first live request.
